...
//	포인터 주소 데이터 타입 'p'를 출력 처리하기 위한 함수.포인터 주소를 long long 형으로 받아오게 된다.
{
	size_t	print_len;
	char	*addr;
	...
	//	포인터 주소 출력에 대한 예외 조건에 부합하지 않는다면 long long 형 가변인자 값을 16진수로 변환하여 addr에 저장한다. 
	addr = applies_to_prec(opt->prec, addr);
	//	정밀도 너비에 따라 가변인자 정수값이 저장되어있는 arg에 정밀도를 적용시킨다.
	addr = ft_strjoin(ft_strdup("0x"), addr);
	//	포인터 주소를 출력할 때 해당 주소는 16진수로 이루어져있다는 것을 나타내기 위해	"0x"를 16진수로 변환된 문자열 addr 앞에 붙여준다.
	addr = applies_to_width(opt, ft_strdup(""), addr);
	//	전체 출력 너비에 따라 패딩 처리하는 과정을 수행한다. 포인터 주소도 마찬가지로 부호 문자를 처리할 필요가 없어 부호 문자열 대신 빈 문자열을 넘겨주었다.
...
//	부호 없는 정수형 타입을 가변인자로 받는 'u', 'x', 'X' 에 대한 출력 처리를 하는 함수
{
	...
	// 데이터 타입이 'u'일 경우 단순히 문자열로 변환하여 arg에 저장;
		...
		arg = ft_tobase_n(n, "0123456789ABCDEF");
	// 데이터 타입이 'x' 또는 'X'일 경우 포인터 주소를 부호 없는 정수형으로 가변인자 값을 받아오기 때문에 부호 없는 정수형 가변인자 값을 16진주로 변환하는 ft_tobase_n 함수를 호출한다.
	else
		return (0);
	// 'X'일 경우의 else if문을 else로 설계하여도 되지만 본인은 명시적이고 가독성이 좋은 코드를 선호하기 때문에 번거롭고 코드가 길어지는 것을 감수하고도 5개의 if 문을 만들었다.
	arg = applies_to_prec(opt->prec, arg);
	//	정밀도 너비에 따라 가변인자 정수값이 저장되어있는 arg에 정밀도를 적용시킨다.
	arg = applies_to_width(opt, ft_strdup(""), arg);
	//	전체 출력 너비에 따라 패딩 처리하는 과정을 수행한다. 부호 없는 정수형을 출력하기 때문에 부호 문자를 처리할 필요가 없어 부호 문자열 대신 빈 문자열을 넘겨주었다.
	print_len = ft_putstr_fd(arg, 1);
	free(arg);
	return (print_len);
}