...
//	padding 문자열을 만들 메모리 공간을 확보하고 '0' 플래그 존재 유무와 너비 값에 따라 패딩 문자열을 만든다.
{
	char	*padding;
	...
	//	'0' 플래그가 존재하지 않았다면 ft_memset 함수를 통해 ' ' 문자를 할당된 메모리 공간에 채운다.
	else
		ft_memset(padding, '0', size);
	//	'0' 플래그가 존재한다면 ft_memset 함수를 통해 '0' 문자를 할당된 메모리 공간에 채운다.
	return (padding);
}
...
{
	char	*result;
		//	'-' 플래그가 존재하지 않는다면 ft_strjoin 함수를 통해 padding 문자열을 기준으로 가변 인자 문자열을 합쳐 우측 정렬을 적용시킨다.
	if (mns_flg == 0)
		result = ft_strjoin(padding, arg);
	else
		result = ft_strjoin(arg, padding);
		//	'-' 플래그가 존재한다면 ft_strjoin 함수를 통해 가변 인자 문자열을 기준으로 padding 문자열을 합쳐 좌측 정렬을 적용시킨다.
	return (result);
}
...
char	*applies_to_width(t_opt *opt, char *sign, char *arg)
{
	...
	//	전체 출력 너비값이 arg 길이와 부호 문자 1byte 길이를 더한 값보다 크면서 정밀도 너비보다도 커야한다.
	{
		...
		//	'minus' 플래그에 따른 부호 위치 조정 및 패딩 문자열을 붙이고나서 '-' 플래그에 따라 좌측 또는 우측 정렬을 적용시킨다.
	}
	else
		arg = ft_strjoin(sign, arg);
	//	if문에서 전체 출력 너비 값 조건에 부합하지 않는다면 단순히 부호와 가변 인자 문자열을 조합한다. 
	return (arg);
}
...
//	result : 최종 변환된 결과를 문자열로 저장할 변수
{
	...
	//	음수에 대한 처리는 구현하지 않음
	{
		...
		{
			if (nbr / base_count != 0)
			//	nbr이 base_count 보다 작은 값일 경우 여기서 문자 추출 및 병합 과정을 수행하지 않고 밑에 if, else 문 다음 과정으로 넘긴다.
			{
				hex = ft_substr(base, nbr / base_count, 1);
				//	ft_substr 함수를 통해 base 문자열에서 나눈 값의 인덱스 문자를 추출하여 hex에 저장
		...
		// 나눈 값이 base_count보다 크거나 같다면 나눈 값을 다시 변환 대상 값으로 재귀 함수 호출
